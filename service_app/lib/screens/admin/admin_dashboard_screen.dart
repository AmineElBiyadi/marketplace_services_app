  ...
  Widget _buildKPIGrid() {
    ...
    return Wrap(
      ...
      children: [
        ...
        _kpiItem('Reservations this month', s.reservationsThisMonth.toString(), LucideIcons.calendarDays, Colors.blue.withOpacity(0.1), Colors.blue, ''),
  ...
  Widget _buildChartsSection() {
    ...
    return Column(
      children: [
          Row(
          children: [
            Expanded(flex: 1, child: _chartCard('Registrations trend (30d)', Screenshot(controller: _inscriptionController, child: _buildInscriptionsChart()))),
            const SizedBox(width: 24),
            if (isPageWide) Expanded(flex: 1, child: _chartCard('Reservations by category', Screenshot(controller: _categoryController, child: _buildCategoryChart()))),
          ],
        ),
        if (!isPageWide) ...[
          const SizedBox(height: 24),
          _chartCard('Reservations by category', Screenshot(controller: _categoryController, child: _buildCategoryChart())),
        ],
        const SizedBox(height: 24),
        Row(
          children: [
            Expanded(flex: 1, child: _chartCard('Free vs Premium distribution', Screenshot(controller: _packController, child: _buildPackChart()))),
            const SizedBox(width: 24),
            ...
            children: [
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: const Text('See all →', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: _primary)),
                ),
              ),
            ],
          ),
  ...
  Widget _buildClaimsList() {
    ...
    return Column(
      children: filtered.map((c) {
        ...
        final rawStatus = c['status'].toString().toUpperCase().replaceAll(' ', '_');
        final displayStatus = const {
          'EN_ATTENTE': 'Pending',
          'PENDING':    'Pending',
          'OUVERT':     'Open',
          'OUVERTE':    'Open',
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  _badge(displayStatus, Colors.amber),
                ],
              ),
            ],
          ),
        ...
        return Padding(
          ...
          child: Row(
            children: [
              ...
              _badge(u['type'] ?? 'Customer', u['type'] == 'Provider' ? _accent : _primary),
            ],
          ),
        );
      }).toList(),
  ...
  Future<void> _exportPdf() async {
    ...
    try {
      ...
      final kpis = [
        ...
        {'label': 'Reservations/Month', 'value': s.reservationsThisMonth.toString()},
      ...
      await AdminExportUtil.exportPageToPdf(
        ...
        subtitle: 'Global summary of platform activity',
        kpis: kpis,
